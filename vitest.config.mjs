...
const proAssetPlugin = {
  name: 'eea-pro-test-asset',
  enforce: 'pre',
  load(id) {
    const filePath = id.split('?')[0];
    if (!filePath.endsWith('.pro')) return null;
    return `export default ${JSON.stringify(path.basename(filePath))};`;
  },
};

export default defineConfig({
  plugins: [proAssetPlugin, jsxInJsPlugin, ...(baseConfig.plugins ?? [])],
  define: {
    __CLIENT__: true,
