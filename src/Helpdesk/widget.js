...
export const RedmineHelpdeskWidgetFactory = ({ widget_button }) => {
  const api = {
    ...
    loading_div: null,
    full_screen:
      window.outerWidth <= 500 && window.outerHeight <= 700 ? true : false,
    width: 400,
    height: 500,
    margin: 20,
    button_size: 54,
    ...
    apply_config: function () {
      ...
      if (this.configuration['color']) {
        ...
        this.loading_div.style.borderTop =
          '8px solid ' + this.configuration['color'];
      }
    ...
    translation: function (field) {
      return this.configuration['translation'] &&
        this.configuration['translation'][field]
        ? this.configuration['translation'][field]
        : null;
    },
    identify: function (field) {
      return this.configuration['identify'] &&
        this.configuration['identify'][field]
        ? this.configuration['identify'][field]
        : null;
    },
    load: function () {
    ...
    decorate_iframe: function () {
      ...
      iframe.style.minHeight = this.full_screen
        ? '100vh'
        : this.px(this.height);
      ...
      if (Object.keys(this.schema.projects).length > 0) {
        ...
        if (this.identify('nameValue')) {
          this.create_form_hidden(
            ...
            'username',
            'form-control',
            this.identify('nameValue'),
          );
        } else {
          this.create_form_text(
            this.form,
            'username',
            'username',
            this.translation('nameLabel') || 'Your name',
            'form-control',
            this.identify('nameValue'),
            true,
          );
        ...
        if (this.identify('emailValue')) {
          ...
          this.create_form_text(
            ...
            'email',
            'email',
            this.translation('emailLabel') || 'Email address',
            'form-control',
            this.identify('emailValue'),
            true,
          );
        ...
        if (this.identify('subjectValue')) {
          ...
          this.create_form_text(
            ...
            'subject',
            'issue[subject]',
            this.translation('subjectLabel') || 'Subject',
            'form-control',
            this.identify('subjectValue'),
            true,
          );
        ...
        this.create_form_area(
          ...
          'description',
          'issue[description]',
          this.translation('descriptionLabel') || 'Ticket description',
          'form-control',
          true,
        );
    ...
    create_projects_selector: function () {
      ...
      if (project_id) {
        this.create_form_hidden(
          ...
          'project_id',
          'project_id',
          'form-control projects',
          project_id,
        );
      } else {
        this.create_form_select(
          this.form,
          'project_id',
          'project_id',
          api.schema.projects,
    ...
    load_project_data: function (project_id, tracker_id) {
      ...
      if (
        ...
        this.create_form_select(
          ...
          'tracker_id',
          'tracker_id',
          this.schema.projects_data[project_id].trackers,
          tracker_id,
          'form-control trackers',
        );
      ...
      this.create_form_submit(
        submit_div,
        this.translation('createButtonLabel') || 'Create ticket',
      );
    ...
    create_form_text: function (
      ...
      field.className = required
        ? field_class + ' required-field'
        : field_class;
      target.appendChild(field);
    },
    create_form_area: function (
      target,
      field_id,
      field_name,
      field_placeholder,
    ...
    create_form_privacy_policy: function (target) {
      ...
      if (typeof api.configuration.privacyPolicy === 'string') {
    ...
    create_attch_link: function (target) {
      if (this.configuration['attachment'] !== false) {
        ...
        attach_link.innerHTML =
          this.translation('attachmentLinkLabel') || 'Attach a file';
        ...
        attach_field.addEventListener('change', function () {
          api.upload_file();
        });
    ...
    upload_file: function () {
      if (
        this.attachment.attributes['data-max-size'] >
    ...
    set_custom_values: function () {
      if (
        this.configuration['identify'] &&
        this.configuration['identify']['customFieldValues']
      ) {
        for (var cf in this.configuration['identify']['customFieldValues']) {
          const custom_field = this.form.querySelector(
            '#issue_custom_field_values_' + this.schema.custom_fields[cf],
          );
          if (custom_field) {
              ...
              case 'SELECT':
                ...
                for (
                  ...
                  if (
                    ...
                    this.create_form_hidden(
                      custom_field.parentElement,
                      custom_field.id,
                      custom_field.name,
                      custom_field.classList.toString(),
                      this.configuration['identify']['customFieldValues'][cf],
                    );
                    custom_field.remove();
                    this.form.querySelector(
    ...
    arrange_iframe: function () {
      ...
      if (!this.full_screen) {
        this.iframe.style.minHeight = this.px(
          this.form.offsetHeight + this.margin,
        );
      } else {
        this.iframe.style.paddingTop = this.px(
          button_margin + button_margin / 3,
        );
      }

      switch (this.configuration['position']) {
        case 'topLeft':
          this.iframe.style.top = this.full_screen
            ? this.px(-this.margin)
            : this.px(button_margin);
          this.iframe.style.left = this.full_screen
            ? this.px(-this.margin)
            : this.px(0);
          break;
        case 'topRight':
          this.iframe.style.top = this.full_screen
            ...
            : this.px(button_margin);
          this.iframe.style.left = this.full_screen
            ? this.px(-window.innerWidth + button_margin)
            : this.px(-this.width + this.button_size);
          break;
        case 'bottomLeft':
          this.iframe.style.top = this.full_screen
            ? this.px(-window.innerHeight + button_margin)
            : this.px(-this.form.offsetHeight - this.margin * 2);
          this.iframe.style.left = this.full_screen
            ? this.px(-this.margin)
            : this.px(0);
          break;
